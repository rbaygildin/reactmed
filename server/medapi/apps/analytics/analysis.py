    ...
    def perform_analysis(self, df, **kwargs):
        ...
        if select_method == 'logit':
            ...
            if class_col is not None:
                df_temp[class_col] = kls_df
            return df_temp
        elif select_method == 'kernel_pca':
            kernel = kwargs.get('kernel', 'linear')
            degree = kwargs.get('degrees', 3)
            index = df_temp.index
            kls_df = None
            if class_col is not None:
                kls_df = df_temp[class_col].to_frame()
                del df_temp[class_col]
            n_cols = len(df_temp.columns)
            gamma = kwargs.get('gamma', 1 / n_cols)
            n_features = kwargs.get('n_features', n_cols // 2)
            if n_features > n_cols:
                raise ValueError("Количество отбираемых переменных "
                                 "не может быть больше количества переменных в исходной выборке")
            if n_features <= 0:
                raise ValueError("Количество отбираемых переменных не может быть неположительным числом!")
            kernel_pca = decom.KernelPCA(n_components=n_features, kernel=kernel, degree=degree, gamma=gamma)
            df_temp = df_temp.apply(standardize)
            if class_col is not None:
                kernel_pca.fit(df_temp, kls_df)
            else:
                kernel_pca.fit(df_temp)
            df_temp = kernel_pca.transform(df_temp)
            df_temp = pd.DataFrame(df_temp, index=index, columns=["KPCA %d" % i for i in range(1, n_features + 1)])
            if class_col is not None:
                df_temp[class_col] = kls_df
            return df_temp
        elif select_method == 'chi2':
            if class_col is None:
                raise ValueError("Категориальная переменная не была задана!")
            kls_df = df_temp[class_col].to_frame()
            del df_temp[class_col]
            ...
            selector.fit(x, y)
            col_ranks = [(df.columns.values[i], rank) for i, rank in enumerate(selector.scores_)]
            col_ranks.sort(key=lambda tup: tup[1])
            df_temp = df[[col[0] for col in col_ranks[0:n_features]]]
            df_temp[class_col] = kls_df
            return df_temp
        elif select_method == 'f_regression':
            if class_col is None:
                raise ValueError("Категориальная переменная не была задана!")
            kls_df = df_temp[class_col].to_frame()
            del df_temp[class_col]
